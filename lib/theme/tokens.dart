...
class EcoTheme {
  ...
  /// Цвет монохромных «оливковых» иконок (заголовки карточек, навигация,
  /// настройки — SVG с зашитым `#555F3B`). В светлой теме отдаёт ровно тот же
  /// олив, что зашит в ассетах, поэтому её вид не меняется; в тёмной — заметно
  /// осветлённый олив, чтобы иконки не сливались с тёмным фоном.
  Color get iconOlive =>
      isDark ? const Color(0xFFAEC194) : const Color(0xFF555F3B);

  /// Базовый тон стеклянных кнопок/чипов/подложек (EcoGlassButton/Chip/Sunken),
  /// поверх которого ползунок «Прозрачность карточек» накладывает альфу. В
  /// светлой теме — белое стекло (как в макете, вид не меняется); в тёмной —
  /// тёмное стекло (в тон карточек), иначе при низкой прозрачности (высокой
  /// альфе) кнопки становились почти сплошь белыми на тёмном фоне.
  Color get glassBase =>
      isDark ? const Color(0xFF3A3E46) : const Color(0xFFFFFFFF);
