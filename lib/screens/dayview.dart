...
class _DayViewScreenState extends State<DayViewScreen> {
        ...
        children: [
          ...
          Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Center(
                child: Text(
                  _isToday ? 'Сегодня' : _ruDate(_selectedDate),
                  style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: EcoColors.sub),
                ),
              ),
                  ...
                  for (final (i, b) in bars.indexed)
                    Padding(
                      ...
                      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                          if (b.head)
                            Text.rich(TextSpan(children: [
                              TextSpan(text: '${b.value.round()}', style: const TextStyle(color: EcoColors.ink)),
                        ...
                        ValueBar(t: t, value: b.value.toDouble(), max: b.max.toDouble(), color: b.color, soft: b.soft, zoneLo: b.zoneLo, zoneHi: b.zoneHi),
                  ...
                  for (final (i, meal) in kMealsByTime.indexed) ...[
                    ...
                    GestureDetector(
                      ...
                      onTap: () => Navigator.of(context).push(MaterialPageRoute(
                        builder: (_) => MealLogScreen(mealKey: meal.key, date: _isToday ? null : _dateKey),
                      )),
                      child: Padding(
                        padding: const EdgeInsets.symmetric(vertical: 10),
                        child: Row(children: [
                          CalBadge(t: t, value: s.mealKcal(meal.key, date: _dateKey), size: 48),
  ...
  static String _ruDate(DateTime d) {
...
class _DayStrip extends StatelessWidget {
  ...
  const _DayStrip({required this.offset, required this.onSelect, required this.store});
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      ...
      children: [
        for (var o = 6; o >= 0; o--) _cell(now.subtract(Duration(days: o)), o),
      ],
    );
  }

  Widget _cell(DateTime date, int o) {
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        child: Column(children: [
          Text(_wd[date.weekday - 1], style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: active ? t.dark : EcoColors.sub)),
          const SizedBox(height: 6),
          MacroRings(size: 32, data: [
            MacroRingData(value: m.carbs, goal: store.carbGoal.toDouble(), color: EcoColors.carb, soft: EcoColors.carbSoft),
            MacroRingData(value: m.fat, goal: store.fatGoal.toDouble(), color: EcoColors.fat, soft: EcoColors.fatSoft),
            MacroRingData(value: m.protein, goal: store.protGoal.toDouble(), color: EcoColors.prot, soft: EcoColors.protSoft),
          ]),
          const SizedBox(height: 6),
          Text('${date.day}', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w800, color: active ? t.dark : EcoColors.ink)),
          // "today" marker dot
          Container(
            ...
            height: 4,
            decoration: BoxDecoration(
              color: o == 0 ? (active ? t.dark : t.olive) : Colors.transparent,
