...
/// Интервал, за который строится совет: пресет («последние N дней до сегодня»,
/// [preset] != null) или произвольный диапазон из календаря ([preset] == null).
/// [start]/[end] — локальные полуночи, конец ВКЛЮЧИТЕЛЬНО.
class AiAdviceRange {
  final AiAdvicePeriod? preset;
  final DateTime start;
  final DateTime end;

  const AiAdviceRange._({
    required this.preset,
    required this.start,
    required this.end,
  });

  /// «Последние N дней до сегодня» — прежнее поведение день/неделя/месяц.
  factory AiAdviceRange.preset(AiAdvicePeriod period, {DateTime? now}) {
    final t = now ?? DateTime.now();
    final startUtc = DateTime.utc(t.year, t.month, t.day)
        .subtract(Duration(days: period.days - 1));
    return AiAdviceRange._(
      preset: period,
  ...
  /// Произвольный диапазон из календаря; порядок концов не важен.
  factory AiAdviceRange.custom(DateTime a, DateTime b) {
    final s = DateTime(a.year, a.month, a.day);
    final e = DateTime(b.year, b.month, b.day);
    return s.isAfter(e)
        ? AiAdviceRange._(preset: null, start: e, end: s)
        : AiAdviceRange._(preset: null, start: s, end: e);
  }

  // Арифметика дней — в UTC: локальные полуночи при сдвигах часового пояса
  // дают неполные сутки, и inDays теряет день.
  ...
  /// Имя периода для бэкенда. Cloud-функция знает только day/week/month и
  /// отклоняет любое другое значение ('Unsupported advice period'), поэтому
  /// произвольный интервал маппим на ближайший по длительности пресет —
  /// фактические границы функция всё равно берёт из start_date/end_date/
  /// period_days, а не из этого имени.
  String get backendPeriodName {
    final p = preset;
    if (p != null) return p.name;
    if (isSingleDay) return AiAdvicePeriod.day.name;
    if (days <= 27) return AiAdvicePeriod.week.name;
    return AiAdvicePeriod.month.name;
  }

  /// Ключи дат yyyy-mm-dd от начала к концу — общий порядок для снапшота,
  /// микронутриентов и счётчика еды.
  List<String> get dateKeys {
    final s = DateTime.utc(start.year, start.month, start.day);
    return [
      for (var i = 0; i < days; i++) AppStore.ymd(s.add(Duration(days: i))),
    ];
  }

  bool sameAs(AiAdviceRange other) =>
      preset == other.preset && start == other.start && end == other.end;
}
...
class AiAdviceService {
  ...
  Future<String> fetchDailyAdvice({
    ...
    // Пользователь отключил ИИ-обработку данных (тумблер согласия) — НИЧЕГО не
    // отправляем в Firebase/DeepSeek. Гейт стоит до сборки снапшота. На главной
    // карточка ИИ при этом скрыта, поэтому это в первую очередь предохранитель.
    if (!store.aiConsent) {
      throw const AiAdviceException(
        AiAdviceErrorKind.generic,
        'AI processing consent is disabled.',
      );
    }
    ...
    try {
      final snapshot = _periodSnapshot(store, language, range);
  ...
  Map<String, Object?> _periodSnapshot(
    ...
    final loggedMeals = range.isSingleDay
        ? [
    ...
    final isCalendarCurrentDay =
        range.isSingleDay && dates.last == AppStore.ymd(now);
    final snapshot = <String, Object?>{
      // Произвольный интервал маппится на легальный пресет (day/week/month) —
      // сервер валидирует это поле по enum; фактические границы он берёт из
      // start_date/end_date + period_days ниже.
      'period': range.backendPeriodName,
